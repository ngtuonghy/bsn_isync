    ...
    <div class="flex items-center gap-5">
      ...
      <div class="flex items-center gap-2 min-w-0 flex-[0.6] pr-4 border-r border-white/5">
        <div class="flex items-center gap-2 p-1 px-2 rounded-xl bg-white/5 border border-white/5 transition-all hover:bg-white/10 flex-1 min-w-0 group/ws">
          ...
          <input v-model="runnerStore.workspaceRoot"
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <Button variant="ghost" class="h-6 text-[10px] px-2.5 hover:bg-white/10 transition-all font-black uppercase tracking-tight rounded-md" @click="runnerStore.pickProjectFolder()">Browse</Button>
            <Button variant="secondary" class="h-7 text-[10px] px-3 font-black uppercase tracking-widest rounded-lg" @click="runnerStore.discoverProjects()">
      ...
      <div class="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div class="flex items-center gap-0.5 bg-white/5 border border-white/10 rounded-xl p-1 shadow-inner ring-1 ring-white/5">
          <div class="flex items-center gap-1.5 px-2.5 py-0.5 bg-background/40 rounded-lg border border-white/5 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all min-w-[130px]">
            ...
            <input v-model="runnerStore.sqlServer"
          ...
          <div class="flex items-center gap-1.5 px-2.5 py-0.5 bg-background/40 rounded-lg border border-white/5 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all min-w-[130px]">
            ...
            <input v-model="runnerStore.sqlDatabase"
                   class="bg-transparent border-0 h-6 w-full text-xs font-mono font-black focus:outline-none placeholder:text-muted-foreground/20"
          ...
          <div class="flex items-center gap-1.5 px-1.5 shrink-0 border-l border-white/10 ml-0.5">
            <div class="relative group/sql-status">
              <div v-if="backlogStore.sqlConnStatus === 'loading'" class="size-2 rounded-full bg-amber-500 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.5)]"></div>
              <div v-else-if="backlogStore.sqlConnStatus === 'success'" class="size-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div>
              <div v-else-if="backlogStore.sqlConnStatus === 'error'" class="size-2 rounded-full bg-destructive shadow-[0_0_8px_rgba(239,68,68,0.5)]"></div>
              <div v-else class="size-2 rounded-full bg-muted shadow-inner opacity-40"></div>

              <!-- Error Tooltip (Simplified) -->
              <div v-if="backlogStore.sqlConnStatus === 'error' && backlogStore.sqlErrorMsg" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 p-2 bg-destructive text-white text-[9px] font-bold rounded-lg shadow-xl opacity-0 group-hover/sql-status:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50">
                {{ backlogStore.sqlErrorMsg.length > 50 ? backlogStore.sqlErrorMsg.substring(0, 50) + '...' : backlogStore.sqlErrorMsg }}
                <div class="absolute top-full left-1/2 -translate-x-1/2 -mt-1 border-4 border-transparent border-t-destructive"></div>
              </div>
            </div>

            <Button variant="ghost" size="icon" class="size-7 rounded-md hover:bg-white/10 transition-all" @click="() => backlogStore.checkSqlConnection(runnerStore.sqlServer, runnerStore.sqlDatabase, runnerStore.sqlSetupPath)" :disabled="backlogStore.sqlConnStatus === 'loading' || !runnerStore.sqlServer || !runnerStore.sqlDatabase" title="Test SQL Connection">
              <RefreshCw class="size-3.5 text-primary/60 hover:text-primary transition-colors" :class="backlogStore.sqlConnStatus === 'loading' ? 'animate-spin' : ''" />
            </Button>
          </div>
        </div>
        ...
        <div class="flex items-center gap-1 bg-white/5 p-1 rounded-xl border border-white/5 shadow-inner">
          <button
            ...
            :class="runnerStore.useWindowsAuth ? 'bg-primary text-primary-foreground shadow-lg scale-105' : 'text-muted-foreground hover:text-foreground opacity-40 hover:opacity-100'"
            @click="runnerStore.useWindowsAuth = true"
          >WIN</button>
          <button
            class="px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all"
            :class="!runnerStore.useWindowsAuth ? 'bg-primary text-primary-foreground shadow-lg scale-105' : 'text-muted-foreground hover:text-foreground opacity-40 hover:opacity-100'"
            @click="runnerStore.useWindowsAuth = false"
          >SQL</button>
        </div>

        <div v-if="!runnerStore.useWindowsAuth" class="w-px h-6 bg-white/10 shrink-0"></div>
        ...
        >
          <div v-if="!runnerStore.useWindowsAuth" class="flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-xl p-1 shadow-inner ring-1 ring-white/5 mr-1">
            <div class="flex items-center gap-1.5 px-2 py-0.5 bg-background/40 rounded-lg border border-white/5 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
              ...
              <input v-model="runnerStore.sqlUser"
            ...
            <div class="flex items-center gap-1.5 px-2 py-0.5 bg-background/40 rounded-lg border border-white/5 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 transition-all">
              ...
              <input v-model="runnerStore.sqlPassword"

...
import { useUiStore, APP_VERSION } from '@/stores/useUiStore';
...
function handleOAuthLogin() {
  backlogStore.startBacklogOAuthLogin();
}
</script>
  ...
  <header class="border-b border-black/5 dark:border-white/5 bg-card sticky top-0 z-50 shadow-sm transition-all duration-300">
    <div class="px-6 py-1.5 flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <BacklogAuthPanel
          class="mr-2"
          v-model:host="backlogStore.backlog.host"
          :apiKey="backlogStore.backlog.apiKey"
          :status="backlogStore.backlog.status"
          :profile="backlogStore.backlog.profile"
          :avatarUrl="backlogStore.backlog.profile ? backlogStore.backlog.userAvatars[backlogStore.backlog.profile.id] : undefined"
          :projects="backlogStore.backlog.projects"
          v-model:selectedProjectKey="runnerStore.backlogProjectKey"
          :error="backlogStore.backlog.error"
          @login-oauth="handleOAuthLogin"
          @logout="backlogStore.handleBacklogLogout"
        />
        <div class="h-4 w-px bg-border mx-1"></div>

        <!-- Update status indicator -->
        <template v-if="uiStore.updateVersion">
          <template v-if="uiStore.isUpdateReady">
            <Button @click="uiStore.applyUpdate()" variant="default" size="sm" class="h-8 gap-1.5 px-3 bg-green-500 hover:bg-green-600 text-white shadow-lg shadow-green-500/20 animate-in fade-in zoom-in duration-300">
              <RefreshCw class="size-3.5 animate-spin-slow" />
              <span class="text-[11px] font-bold">Restart Now</span>
            </Button>
          </template>
          <template v-else>
            <Button @click="uiStore.downloadUpdate()" :disabled="uiStore.isUpdateDownloading" variant="secondary" size="sm" class="h-8 gap-1.5 px-3 transition-all">
              <ArrowUpCircle class="size-3.5" :class="uiStore.isUpdateDownloading ? 'animate-bounce' : ''" />
              <span class="text-[11px] font-bold">{{ uiStore.isUpdateDownloading ? 'Downloading...' : `Update Now (v${uiStore.updateVersion})` }}</span>
            </Button>
          </template>
          <div class="h-4 w-px bg-border mx-1"></div>
        </template>
        ...
        <div v-if="uiStore.envStatus.some((x) => !x.found)" class="relative group/env mr-1">
          <Button variant="ghost" size="icon" class="h-7 w-7 transition-all hover:bg-zinc-100 dark:hover:bg-white/5 rounded-lg"
                  :class="uiStore.envStatus.some((x) => !x.found) ? 'text-amber-500' : 'text-green-500'">
            <ShieldCheck v-if="uiStore.envStatus.every((x) => x.found)" class="size-4" />
          ...
          <div class="absolute right-0 top-full mt-2 w-72 bg-card/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] opacity-0 invisible group-hover/env:opacity-100 group-hover/env:visible transition-all z-50 p-4 scale-95 group-hover/env:scale-100 origin-top-right ring-1 ring-white/10">
            <div class="flex items-center justify-between mb-4">
              ...
              <Button variant="ghost" size="icon" class="h-6 w-6 rounded-md hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all" @click="uiStore.checkEnv()" title="Re-check environment">
                <RefreshCw class="size-3" />
              </Button>
            </div>

            <div class="space-y-2">
              <div v-for="tool in uiStore.envStatus" :key="tool.name"
            ...
            <div v-if="uiStore.envStatus.some((x) => !x.found)" class="mt-4 p-2.5 rounded-xl bg-amber-500/5 border border-amber-500/10">
        ...
        <div class="flex items-center gap-1.5 bg-background/50 border border-border/50 p-1 rounded-full shadow-inner mr-2">
          <label class="flex items-center gap-1.5 px-2.5 py-1 rounded-full cursor-pointer transition-all select-none" :class="runnerStore.forceUnicode ? 'bg-primary text-primary-foreground shadow-sm' : 'hover:bg-muted text-muted-foreground'">
            <input type="checkbox" v-model="runnerStore.forceUnicode" class="sr-only" />
            <span class="text-[9px] font-black uppercase tracking-wider">Shift-JIS</span>
          </label>
          <div class="w-px h-3 bg-border/50 mx-0.5"></div>
          <label class="flex items-center gap-1.5 px-2.5 py-1 rounded-full cursor-pointer transition-all select-none" :class="runnerStore.autoWatchTargetTest ? 'bg-blue-500 text-white shadow-sm' : 'hover:bg-muted text-muted-foreground'">
            <input type="checkbox" v-model="runnerStore.autoWatchTargetTest" class="sr-only" />
            <span class="text-[9px] font-black uppercase tracking-wider">Watch Test</span>
          </label>
          <label class="flex items-center gap-1.5 px-2.5 py-1 rounded-full cursor-pointer transition-all select-none" :class="runnerStore.autoWatchTargetRun ? 'bg-green-500 text-white shadow-sm' : 'hover:bg-muted text-muted-foreground'">
            <input type="checkbox" v-model="runnerStore.autoWatchTargetRun" class="sr-only" />
        ...
        <div class="relative group/sys-settings ml-1">
          ...
          <div class="absolute right-0 top-full mt-2 w-48 bg-card/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl opacity-0 invisible group-hover/sys-settings:opacity-100 group-hover/sys-settings:visible transition-all z-50 p-2 scale-95 group-hover/sys-settings:scale-100 origin-top-right">
            <div class="flex flex-col gap-1">
              <button @click="terminalStore.toggleTheme()" class="flex items-center gap-2 px-3 py-2 text-xs font-medium hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground w-full text-left transition-colors">
                <Moon v-if="!uiStore.isDarkMode" class="size-3.5" />
                <Sun v-else class="size-3.5 text-yellow-500" />
                <span>Theme ({{ uiStore.isDarkMode ? 'Dark' : 'Light' }})</span>
              </button>
              <button @click="uiStore.isTerminalHistoryEnabled = !uiStore.isTerminalHistoryEnabled" class="flex items-center gap-2 px-3 py-2 text-xs font-medium hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground w-full text-left transition-colors">
                <History class="size-3.5" :class="uiStore.isTerminalHistoryEnabled ? 'text-primary' : ''" />
                <span>Terminal History: {{ uiStore.isTerminalHistoryEnabled ? 'On' : 'Off' }}</span>
              </button>
              <button @click="uiStore.isNotificationEnabled = !uiStore.isNotificationEnabled" class="flex items-center gap-2 px-3 py-2 text-xs font-medium hover:bg-muted rounded-lg text-muted-foreground hover:text-foreground w-full text-left transition-colors">
                <Bell v-if="uiStore.isNotificationEnabled" class="size-3.5 text-primary" />
                <BellOff v-else class="size-3.5" />
                <span>Notifications: {{ uiStore.isNotificationEnabled ? 'On' : 'Off' }}</span>
              </button>
              ...
              <button v-if="uiStore.envStatus.every((x) => x.found)" @click="uiStore.checkEnv()" class="flex items-center gap-2 px-3 py-2 text-xs font-medium hover:bg-muted rounded-lg text-green-500 hover:text-green-600 w-full text-left transition-colors">
